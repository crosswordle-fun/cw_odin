...
app_handle_global_shortcuts :: proc(state: ^GameState) {
...
app_present_render_target :: proc(render_target: rl.RenderTexture, theme: Theme) {
	...
	rl.BeginDrawing()
	rl.ClearBackground(theme.canvas)
...
app_run_frame :: proc(
	...
	if state.should_quit do return false
	...
	app_present_render_target(render_target, ctx.theme)
	return true
}
